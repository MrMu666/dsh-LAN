	...
	factory: (require) => {
		...
		(function mobileAdapt() {
			...
			const ADAPT_CSS = [
				...
				// both the `open-in-app` app chooser (文件资源管理器 / VS Code /
				// Git Bash) and `session-log-export` register into THIS slot, so the
				// whole utilities cluster goes away on phones. The app chooser is
				// additionally hidden for every non-local browser below.
