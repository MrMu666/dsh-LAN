	...
	factory: (require) => {
		...
		(function mobileAdapt() {
			...
			const ADAPT_CSS = [
				...
				// v80: hard ceiling on the model trigger so a long model name can
				...
				// button). 140px is the largest reservation that still keeps a REAL
				// model name ("DeepSeek V4.1 Flash (CC)", 137px) unclipped at 390px
				// while a long one stays clear of the circle; measured sweep, gap to
				// the send circle at 390px: 160px->34 (clips the real name),
				// 150px->24 (clips), 140px->19 real / 14 long (no clip), 130px->4,
				// 120px->-6 (overlap). Narrower viewports necessarily ellipsize the
				// model name — that is the intended trade, the send button wins.
				"[class$=\"_composerSeat\"] [class$=\"_trailing\"] [class$=\"_trigger\"]{max-width:calc(100cqw - 140px)}",
