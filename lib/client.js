	...
	factory: (require) => {
		...
		(function mobileAdapt() {
			...
			const ADAPT_CSS = [
				...
				// ("对话 / 轨迹"), right-aligned. Hidden in the original spot so
				// React re-renders don't flicker them back between re-seats.
				"[class$=\"_header\"] [class$=\"_titleCluster\"] [class$=\"_headerActions\"]{display:none}",
				// the official header keeps a 78px right padding on phones, so
				// the tabs row ends 78px short of the viewport edge — stretch it
				// to end 20px from the edge (78-20) so the seated actions truly
				// sit flush against the right edge (verified 360/390/480).
