	...
	factory: (require) => {
		...
		(function mobileAdapt() {
			...
			const ADAPT_CSS = [
				...
				"#dshLanWhale{position:fixed;top:calc(8px + env(safe-area-inset-top));left:calc(8px + env(safe-area-inset-left));z-index:2147482999;width:38px;height:38px;min-width:38px;padding:0;border-radius:11px;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-primary);display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 12px rgba(0,0,0,.35)}",
				"#dshLanWhale svg{width:23px;height:17px;display:block}",
				...
				// composer card edge, tighter line-height and letter-spacing;
			...
			function ensureWhale() {
				...
				whaleEl.addEventListener("pointermove", (e) => {
					...
					whaleEl.style.left = Math.min(Math.max(4, drag.left + dx), window.innerWidth - 42) + "px";
					whaleEl.style.top = Math.min(Math.max(4, drag.top + dy), window.innerHeight - 42) + "px";
					e.preventDefault();
					e.stopPropagation();
				});
				const endWhaleDrag = () => {
